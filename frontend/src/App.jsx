...
//     state_commodity: { label: "State + commodity fallback", tone: "warn", desc: "Market-level match missing; using wider pattern." },
//     commodity_only: { label: "Commodity fallback", tone: "warn2", desc: "Using commodity patterns without market/state precision." },
...
//   const scoreMap = { exact: 0.9, latest_in_series: 0.7, state_commodity: 0.55, commodity_only: 0.4, date_only: 0.2 };
...
//           <path d={arcPath(Math.PI, 0)} stroke="rgba(148,163,184,0.55)" strokeWidth="12" fill="none" strokeLinecap="round" />
//           <path d={arcPath(Math.PI, angle)} stroke="rgba(16,185,129,0.85)" strokeWidth="12" fill="none" strokeLinecap="round" />
//           <circle cx={x} cy={y} r="6" fill="rgba(2,132,199,0.95)" />
//         </svg>
...
//   useEffect(() => {
//     (async () => {
//       try { await api.get("/health"); setApiLive(true); } catch { setApiLive(false); }
//     })();
//   }, []);
...
//       try {
//         const res = await api.get("/meta/top-markets", { params: { state: selectedState, commodity: selectedCommodity, k: 5 } });
...
//     const payload = { state: selectedState, market: selectedMarket, commodity: selectedCommodity, arrival_date: arrivalDate };

//     try {
//       const res = await api.post("/predict", payload);
//       setResult(res.data);
//       // scroll to results like a real website
...
//               <div className={soft ? "text-xs text-slate-600" : "text-xs text-slate-400"}>A modern agri-fintech price intelligence tool</div>
//             </div>
//           </div>
...
//         {metaError && (
//           <div className={soft ? "mb-6 rounded-2xl p-4 border bg-rose-50 border-rose-200 text-rose-700" : "mb-6 rounded-2xl p-4 border bg-rose-500/10 border-rose-400/20 text-rose-200"}>
//             <div className="font-semibold">Meta issue</div>
//             <div className="text-sm mt-1">{metaError}</div>
//           </div>
//         )}

//         <div className={soft ? "rounded-[28px] border border-slate-200 bg-white/75 backdrop-blur shadow-sm overflow-hidden" : "rounded-[28px] border border-slate-700 bg-slate-900/40 shadow-sm overflow-hidden"}>
//           <div className={soft ? "px-6 py-10 md:px-10 bg-gradient-to-r from-emerald-100/60 via-white to-amber-100/50" : "px-6 py-10 md:px-10 bg-gradient-to-r from-sky-500/10 via-slate-900/0 to-emerald-500/10"}>
//             <div className="max-w-3xl">
//               <h1 className={soft ? "text-3xl md:text-5xl font-extrabold tracking-tight text-slate-900" : "text-3xl md:text-5xl font-extrabold tracking-tight text-white"}>
...
//                 <Select styles={selectStyles} options={toOptions(states)} value={stateValue} onChange={(opt) => setSelectedState(opt?.value || "")}
//                   placeholder={metaLoading ? "Loading..." : "Search state..."} isClearable isDisabled={metaLoading}
...
//                 <Select styles={selectStyles} options={toOptions(markets)} value={marketValue} onChange={(opt) => setSelectedMarket(opt?.value || "")}
//                   placeholder={selectedState ? "Search market..." : "Select state first"} isDisabled={!selectedState || metaLoading}
//                   isClearable noOptionsMessage={() => (!selectedState ? "Select state first" : "No markets")}
//                 />
//               </div>
//               <div>
//                 <div className={soft ? "text-xs mb-1 text-slate-600" : "text-xs mb-1 text-slate-300"}>Commodity</div>
//                 <Select styles={selectStyles} options={toOptions(commodities)} value={commodityValue} onChange={(opt) => setSelectedCommodity(opt?.value || "")}
//                   placeholder={metaLoading ? "Loading..." : "Search commodity..."} isClearable isDisabled={metaLoading}
...
//                 <input
//                   type="date"
//                   className={soft ? "w-full h-[46px] rounded-2xl px-3 border border-slate-200 bg-white/80 text-slate-900" : "w-full h-[46px] rounded-2xl px-3 border border-slate-700 bg-slate-800 text-white"}
//                   value={arrivalDate}
...
//               <button
//                 onClick={handleReset}
//                 className={soft ? "px-4 py-2.5 rounded-2xl border border-slate-200 hover:bg-white text-sm" : "px-4 py-2.5 rounded-2xl border border-slate-700 hover:bg-slate-900/40 text-sm"}
//               >
//                 Reset
//               </button>
...
//                   <>
//                     <button
//                       onClick={() => downloadJSON(result, `forecast_${result.state}_${result.market}_${result.commodity}_${result.arrival_date}.json`.replaceAll(" ", "_"))}
//                       className={soft ? "px-4 py-2.5 rounded-2xl border border-slate-200 hover:bg-white text-sm" : "px-4 py-2.5 rounded-2xl border border-slate-700 hover:bg-slate-900/40 text-sm"}
//                     >
//                       Download JSON
//                     </button>
...
//             {/* How it works strip */}
//             <div className={soft ? "mt-8 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm" : "mt-8 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm"}>
...
//             <div className={soft ? "rounded-[28px] border border-slate-200 bg-white/75 backdrop-blur p-6 shadow-sm" : "rounded-[28px] border border-slate-700 bg-slate-900/40 p-6 shadow-sm"}>
...
//         {/* About section (major-project vibe) */}
